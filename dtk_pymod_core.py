...
def conceive_baby_callback(individual_id):
    ...
    for human in human_pop:
        ...
        if fertility:
            nd.update_node_stats(
                (mcw, 0, gi.is_possible_mother(hum_id), 0))  # mcw, infectiousness, is_poss_mom, is_infected
        """
...
def do_vitaldynamics_update(human_pop, graveyard, contagion, census_cb=None, death_cb=None):
    ...
    for human in human_pop:
        ...
        gi.update2(hum_id)  # this should do exposure; possible optimization would be to
        # skip this entirely if zero contagion
